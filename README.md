...
The human is also enough on their own. The interactive sign-in asks for the delegated form of
every permission the service app is granted, so a Global Administrator who would rather not leave
an application registration behind can seed and tear down as themselves; see
...
The device-code sign-in asks for the delegated form of every permission in
`Providers\Entra\Data\EntraServiceAppPermissions.csv` rather than `.default`, so the first sign-in
shows one consent screen listing exactly what the module needs, and the session holds those
scopes from then on. Two names differ from the application permissions: `Device.ReadWrite.All`
...
refuses. `-Scope` overrides the list, and `-Scope '.default'` asks for nothing beyond what the
client was already consented for, which was the behaviour before the list existed.
