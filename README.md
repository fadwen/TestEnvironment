...
| [`FreeIPA`](Providers/FreeIPA/README.md) | a service account password, bootstrapped once from an administrator's credential | ~970 objects across twenty-eight types: users in every lifecycle state, some authenticating through a seeded RADIUS proxy or external identity provider, groups and host groups with member managers, hosts that resolve in the seed's own DNS zones, then HBAC, sudo, roles, policies, services, ID views, tokens, automember, automount, SELinux and certificate mapping over them, and certificates the realm's own CA issued |
