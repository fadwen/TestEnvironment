function New-TestEnvironment {
    ...
    .OUTPUTS
        TestEnvironmentResult, or whatever the active provider returns.

    .EXAMPLE
        PS> New-TestEnvironment
    ...
    dynamicparam {
        return Get-TestProviderParameter -CommandName ('New-{0}Environment' -f $script:ActiveProvider)
    }
    ...
    process {
        $target = 'New-{0}Environment' -f $script:ActiveProvider
