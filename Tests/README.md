...
| `Providers\AD\SeedLookupScope.Tests.ps1` | That every directory search the user and group steps run inside a background job is scoped to the seed OU, apart from the domain-wide check that an account name is free, so a seeded group can never take in an account the seed did not create |
