function Get-FreeIPADataPath {
    <#
    .SYNOPSIS
        Returns the FreeIPA provider's Data folder
    ...
    .EXAMPLE
        PS> Import-Csv -Path (Join-Path (Get-FreeIPADataPath) 'FreeIPAUsers.csv') -Encoding UTF8
    ...
    $dataPath = $script:TestEnvironmentProvider['FreeIPA'].DataPath
    if (-not (Test-Path -LiteralPath $dataPath)) {
        throw "The FreeIPA provider's Data folder is missing from $dataPath. The module is incomplete."
    }
    return $dataPath
}
