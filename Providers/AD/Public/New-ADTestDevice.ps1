        ...
        try {
            ...
            foreach ($batch in $deviceBatches) {
                ...
                while ($script:ProcessingJobs.Count -ge $ThrottleLimit) {
                    ...
                    # received, and its tally disappeared while its computers sat in the
                    # directory. That is what reported 569 of 688 devices created, with none
                    ...
                    if ($finished) {
                        foreach ($job in $finished) {
                            ...
                            if ($job.State -ne 'Completed') {
                                $script:Errors += "Device batch $($job.Name) ended in state $($job.State)"
                                continue
                            }
