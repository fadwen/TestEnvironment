...
for the delegated form of every permission in `Data\EntraServiceAppPermissions.csv`,
...
`Organization.Read.All`; `Data\EntraServiceAppPermissions.csv` is the
...
`New-TestEnvironment`, `Remove-TestEnvironment` and `Get-TestEnvironmentReport` mirror the Entra
provider's own parameters once it is connected. `-Skip` and `-Keep` take the same names:
`AdministrativeUnits`, `Users`, `Groups`, `Licenses`, `Devices`, `Applications`, `NamedLocations`,
`ConditionalAccessPolicies`, `Containment`.
...
The shared teardown rules - ownership proven first, rights judged before any prompt, `-WhatIf`
beating `-Force` - are described in the [module README](../../README.md#-teardown). What is
specific to Entra is the order, and what happens after a delete.
