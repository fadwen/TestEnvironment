...
foreach ($adGroup in $adGroups) {
    ...
    if ($usersByDepartment.ContainsKey($adGroup.Category)) {
        ...
        $members = @($allUsers.Key | Sort-Object -Property @{ Expression = { Get-StableHash "$key|$_" } }, @{ Expression = { $_ } } | Select-Object -First $take)
    }
