...
HelpUri: https://github.com/fadwen/TestEnvironment/blob/main/docs/TestEnvironment/New-TestServiceApp.md
Locale: en-US
Module Name: TestEnvironment
ms.date: 09 09 2026
PlatyPS schema version: 2024-05-01
title: New-TestServiceApp
---
...
```
New-TestServiceApp [-WhatIf] [-Confirm]
```
...
### Example 2: Stores the new credential in the SecretStore instead of a file

```powershell
New-TestServiceApp -UseSecretStore
```

Output: The app created and the vault the credential went to.

Use case: A shared machine, where a private key on disk under the profile is the wrong place for it.

### Example 3: Names the Entra app and shortens its certificate lifetime

```powershell
New-TestServiceApp -DisplayName LabSeeder -CertificateValidityDays 90 -PassThru
```

Output: The application, its service principal and the certificate thumbprint.

Use case: A tenant with a policy on credential lifetime. These parameters are the Entra provider's;
Okta's bootstrap takes -Label and -KeySize instead.
...
The provider's description of the app it created and where the credential was stored, when -PassThru is supplied.
